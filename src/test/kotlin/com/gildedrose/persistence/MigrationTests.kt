...
class MigrationTests {
    ...
    @Test
    fun `can read new items`(approver: Approver) {
        val inputLines = stockList.lines()
        val outputLines: List<String> = migrate(inputLines)
        val readStockList = outputLines.asSequence().toStockList().onFailure { error(it.reason) }
        approver.assertApproved(readStockList.toLines().joinToString("\n"))
    }
}
...
private fun migrate(inputLines: List<String>): List<String> {
    ...
    val newItems: List<Item> =
        stockList.map { it.withID(ids).also { newItem -> ids.add(newItem.id!!) } }
...
private fun Item.withID(ids: Set<ID<Item>>): Item {
    for (i in 1..100) {
        val candidate = ID<Item>(initialsFrom(name) + i)!!
        if (candidate !in ids) return this.copy(id = candidate)
    }
    error("Used by my numbers")
}

fun initialsFrom(name: NonBlankString): String = name.split(" ")
    .map { it[0] }
    .joinToString("")
    .uppercase()

val stockList = """
    # LastModified: 2023-09-13T07:43:47.787586Z
